      ...
      {!isEditing ? (
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="flex space-x-3 pt-2">
            <button type="submit" className="px-4 py-2 bg-emerald-600 text-white rounded-lg text-xs font-semibold hover:bg-emerald-700">
              Save Changes
            </button>
            <button 
              type="button" 
              onClick={() => { setIsEditing(false); setFormData(teacher); }} 
              className="px-4 py-2 bg-slate-200 text-slate-700 rounded-lg text-xs font-semibold hover:bg-slate-300"
            >
              Cancel
            </button>
          </div>
